...
def train():
    ...
    train_npz_filepath = config.train_npz_filepath
    train_dataset_npz = np.load(train_npz_filepath)
    train_batch_size = config.train_batch_size
    train_dataset = Dataset(img_size, data_augmentation = True)
    train_dataset.update(train_dataset_npz['observations'].shape[0], torch.from_numpy(train_dataset_npz['observations']).float(), train_dataset_npz['labels'])
    train_loader = DataLoader(train_dataset, batch_size=train_batch_size, shuffle=True)
    
    valid_npz_filepath = config.valid_npz_filepath
    valid_dataset_npz = np.load(valid_npz_filepath)
    valid_batch_size = config.valid_batch_size
    valid_dataset = Dataset(img_size)
    valid_dataset.update(valid_dataset_npz['observations'].shape[0], torch.from_numpy(valid_dataset_npz['observations']).float(), valid_dataset_npz['labels'])
